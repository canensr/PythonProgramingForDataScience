...
num = ["NUM_"+column.upper() if df[column].dtype != "O" else column.upper() for column in df.columns]
num
...
flag = [column.upper()+"_FLAG" if "no" not in column else column.upper() for column in df.columns]
flag
...
ff=[]
df2 = [ff.append(col) for col in df.columns if col not in og_list]
new_Df=[df[ff]]
# # Notlar:
